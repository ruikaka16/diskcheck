...
public class ReadLinuxFile extends JFrame{
	...
	private void initComponents() {
		...
		jButton2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				// TODO Auto-generated method stub
				try {
					WriteLinuxInfo("168.100.102.42", "hundsun", "hundsun");
	...
	public void WriteLinuxInfo(String ip, String username, String password)
		...
		try {
			...
			insertLinuxCommdToDatabase("168.100.102.42");
	...
	public void insertLinuxCommdToDatabase(String ip) {
		...
		File txtfile = new File("c://42.txt");
		...
		try {
			...
			while (null != (str = bf.readLine())) {
				if (!str.trim().isEmpty()) {
					String s[] = str.split(" ");
					String sql = "insert into update_sofilelist (ip,file_name,file_size,file_time,calc_time,system_type) values('"+ip+"','"+s[8]+"','"+s[4]+"','"+s[7]+"-"+s[5]+"-"+s[6]+"',curdate(),0)";
